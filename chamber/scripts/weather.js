...
async function fetchWeather() {
    const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${chamberLocation.city},${chamberLocation.countryCode}&units=metric&appid=${apiKey}`);
    ...
    const forecastResponse = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${chamberLocation.city},${chamberLocation.countryCode}&units=metric&appid=${apiKey}`);
    ...
    for (let i = 0; i < 3; i++) {
        ...
        forecastItem.textContent = `Day ${i + 1}: ${forecastTemp}°C, ${forecastDescription}`;
